...
int   debug_enable = 0;
static char *debug_buf = (char *)CONFIG_NORMAL_DEBUG_BASE;
/*
******************************************************************************************************************
*
...
void show_tick(void)
{
	uint time_msec, time_sec, time_rest;
	char printbuffer[64];
	int count;

	time_msec = (uint)get_sys_ticks();
	time_sec =  time_msec/1000;
	time_rest = time_msec%1000;

	/* For this to work, printbuffer must be larger than
	 * anything we ever want to print.
	 */
	count = sprintf(printbuffer, "[     %d.%d]", time_sec, time_rest);

	/* Print the string */
	if (debug_enable)
		puts(printbuffer);
	else {
		memcpy(debug_buf, printbuffer, count);
		debug_buf += count;
		*debug_buf = '\0';
	}
...
{
	puts((char *)CONFIG_NORMAL_DEBUG_BASE);
}

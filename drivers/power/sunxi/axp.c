...
int axp_set_supply_status_byname(char *pmu_type, char *vol_type, int vol_value, int onoff)
{
	...
	if(sunxi_axp_dev[axp_index] != NULL)
		ret = sunxi_axp_dev[axp_index]->set_supply_status_byname(vol_type, vol_value, onoff);

	printf("axp_set_supply_status_byname(%s,%s,%d,%d)=%d [%x]\n", pmu_type, vol_type, vol_value, onoff, ret, (uint32_t)sunxi_axp_dev[axp_index]);
	return -1;
}

...
static int mmc_startup(struct mmc *mmc)
{
	...
	MMCDBG("%s -- 0x%02x-%02x-%02x-%02x-%02x\n", mmc->block_dev.product,
	...
	MMCINFO("MMC%d: %s %s %dMHz %d bit %ld MB\n",
		mmc->cfg->host_no, mmc_ver, spd_name[mmc->speed_mode], mmc->clock/1000000, mmc->bus_width, mmc->block_dev.lba>>11);
...
int mmc_init_boot(struct mmc *mmc)
{
	...
	MMCDBG("erase_grp_size      : 0x%xWrBlk*0x%x=0x%x Byte\n",
    ...
    MMCDBG("secure_feature      : 0x%x\n", mmc->secure_feature);
    MMCDBG("secure_removal_type : 0x%x\n", mmc->secure_removal_type);
    if (!IS_SD(mmc)) {

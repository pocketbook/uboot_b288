...
{	
	u8 eink_bits = 0;
	if (EINK_BIT_5 == param->eink_bits) {
		eink_bits = 2;
	} else if (EINK_BIT_4 == param->eink_bits) {
		eink_bits = 1;
	} else {
		eink_bits = 0;
	}
	eink_config(eink_bits, param->eink_mode);
	return 0;
}
...
int disp_al_lcd_get_clk_info(u32 screen_id, struct lcd_clk_info *info, disp_panel_para * panel)
{
	...
	if((panel->lcd_dclk_freq %1000) >500)
	{
		tmp = panel->lcd_dclk_freq /1000 + 1;
	}
	else
	{
		tmp = panel->lcd_dclk_freq /1000 ;
	}
#else
	tmp = panel->lcd_dclk_freq ;
#endif

	if (tmp < 10) {
		tcon_div = 30;
		lcd_div = 1;
	} else if (tmp < 40 && tmp >= 10) {

...
volatile int diplay_finish_flag = 0;
...
static unsigned int wavedata_buf_size;
...
static void __eink_get_sys_config(u32 disp, struct eink_init_param *eink_param)
{
	...
	strcpy(eink_param->wavefile_path, "wavefile\\default.bin");
}
...
s32 eink_enable(struct disp_eink_manager* manager)
{
	...
	if (first_enable) {
		...
		ret = disp_al_init_eink_ctrl_data_8(manager->disp,
			((unsigned long)(manager->private_data->wavedata_ring_buffer.wavedata_vaddr[0])),
			&param.timing, 0);

		for (i=1; i<WAVE_DATA_BUF_NUM; i++) {
			memcpy(manager->private_data->wavedata_ring_buffer.wavedata_vaddr[i],
							manager->private_data->wavedata_ring_buffer.wavedata_vaddr[0],
							wavedata_buf_size
			);
		}
...
*/
		printf("disp_al_init_eink_ctrl_data_8 ok\n");
...
{	
	...
	*/
	...
	for(disp=0; disp<MAX_EINK_ENGINE; disp++) {
		...
		tps65185_temperature_get(&(manager->eink_panel_temperature));
		manager->eink_panel_temperature = 28;
		printf("%s: get temperature=%d degree from tps65185\n", __func__, manager->eink_panel_temperature);
		#else 
		manager->eink_panel_temperature = 28;
		printf("%s: set default temperature to %d degree\n", __func__, manager->eink_panel_temperature);
		#endif
		/* functions for debug */
		...
		for (wd_buf_id = 0; wd_buf_id < WAVE_DATA_BUF_NUM; wd_buf_id++) {
			...
			__inf("wavedata id=%d, virt-addr=%p, phy-addr=%p,wavedata_buf_size=%x\n",
			wd_buf_id,
...
#if 0
#ifdef CONFIG_TPS65185_VCOM
		/*set default vcom voltage*/
	    ret = tps65185_vcom_set(-3000);		//unit is mv		
		if (0 != ret) {
			__wrn("%s: set vcom fail, ret=%d\n", __func__, ret);
		}
#endif
#endif

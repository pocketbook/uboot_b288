...
int update_for_boot0(char *boot0_name, int storage_type)
{
	...
	length = fread(boot0_buf, 1, sizeof(boot0_buf), boot0_file);
	if(length < 512)
	{
		goto _err_boot0_out;
	}
	...
	int align = boot0_head->boot_head.length;
	length = (length + align - 1) & ~(align - 1);
	boot0_head->boot_head.length = length;
	printf("align %d, boot0 length %d\n", align, length);

...
*/
	...
	else if(strncmp(cmd, "phy_read", strlen("phy_read")) == 0)
	{
		...
		return ret == 0 ? 1 : 0;
	} else if(!strncmp(cmd, "p_read", strlen("p_read"))) {
		u32 start_block;
		u32 size = 0;
		u32 offset;
		if (argc != 6)
		...
		addr = (ulong)simple_strtoul(argv[2], NULL, 16);
		offset = (u32)simple_strtoul(argv[4], NULL, 16);
		size = (u32)simple_strtoul(argv[5], NULL, 16);
		//printf("a = %lx; p = %s; o = %x; s = %x\n", addr, part_name, offset, size);
		start_block = sunxi_partition_get_offset_byname((const char *)part_name);
		if(!start_block) {
			printf("cant find partition named %s\n", (char *)part_name);
			return 0;
		}
		u32 psize = sunxi_partition_get_size_byname((const char *)part_name);
		printf ("find partition %s: start = %u (%x); size = %u (%x)\n", (char *)part_name, start_block, start_block, psize, psize);
		if (offset + size > psize) {
			printf("read area is outside layout\n");
			return 0;
		}

		ret = sunxi_flash_read(start_block + offset, size, (void *)addr);

		tick_printf("sunxi flash p_read :offset %x, %d *512 bytes %s\n", start_block + offset, size,
				ret ? "OK" : "ERROR");
...
U_BOOT_CMD(
	...
	"\ncommand \"p_read\" read by offset of partition in sectors\n"
	"            cmd    dest     part  offset size\n"
	"sunxi_flash p_read 42200000 pbenv 2048 4000"
);

...
//#define   BATTERY_MIN_VOLTAGE      4100
...
{
	u8 reg_vbat, reg_istat, reg_pwroff;
	int vbat, inp;
	int ledon = 1;
	int delaycnt = 1;

	i2c_init(CONFIG_SYS_I2C_SPEED,CONFIG_SYS_I2C_SLAVE);
	axp_i2c_write(AXP22_ADDR, 0x33, (1 << 7) | (2 << 5) | (0 << 0)); // charge on, limit to 300mA
	__msdelay(20); // wait for axp voltage measure cycle
	while (1) {
		set_led(ledon);
		int r = axp_i2c_read(AXP22_ADDR, 0x78, &reg_vbat);
		r |= axp_i2c_read(AXP22_ADDR, 0x00, &reg_istat);
		if (r) {
			printf("AXP read error!\n");
			return 0;
		}
		vbat = ((reg_vbat << 4) * 11) / 10;
		...
		if (! inp) {
			printf("VBAT=%d, poweroff\n", vbat);
			__msdelay(100);
			if (axp_i2c_read(AXP22_ADDR, 0x32, &reg_pwroff) == 0) {
				axp_i2c_write(AXP22_ADDR, 0x32, reg_pwroff | 0x80);
			}
		}
		__msdelay(250);
		ledon = 1 - ledon;
